...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("generate-fields")
    .addEventListener("click", generatePlayerFields);
  document
  ...
  document
    .getElementById("show-tournament")
    .addEventListener("click", showTournament);
});
...
async function verifyAtCoderid(e) {
  ...
  const atcoderId = document
    .getElementById(`player-id-${playerNum}`)
    .value.trim();
  ...
  try {
    ...
    const response = await fetch(
      `https://kenkoooo.com/atcoder/atcoder-api/v3/user/submissions?user=${atcoderId}&from_second=0`
    );
...
async function startTournament() {
  ...
  matchesContainer.innerHTML =
    '<div class="loading-message">Setting up matches...</div>';
  ...
  try {
    ...
    matchesContainer.innerHTML =
      '<div class="error-message">Failed to set up matches. Please try again.</div>';
  }
}
...
async function getUnsolvedProblem(player1Id, player2Id) {
  try {
    ...
    const player1Response = await fetch(
      `https://kenkoooo.com/atcoder/atcoder-api/v3/user/submissions?user=${player1Id}&from_second=0`
    );
    const player2Response = await fetch(
      `https://kenkoooo.com/atcoder/atcoder-api/v3/user/submissions?user=${player2Id}&from_second=0`
    );
    ...
    // Fallback to a random beginner A problem if no common unsolved problems found
    const randomContestId =
      beginnerContests[Math.floor(Math.random() * beginnerContests.length)];
    return {
...
function displayMatches() {
  ...
  matches.forEach((match) => {
    ...
    player1Status.textContent = match.player1Solved
      ? `Solved in ${formatTime(match.player1Time)}`
      : "Not solved yet";
    ...
    player2Status.textContent = match.player2Solved
      ? `Solved in ${formatTime(match.player2Time)}`
      : "Not solved yet";
...
async function checkSubmissions() {
  ...
  const submissionsPromises = Array.from(playerIds).map(async (playerId) => {
    try {
      const response = await fetch(
        `https://kenkoooo.com/atcoder/atcoder-api/v3/user/submissions?user=${playerId}&from_second=0`
      );
  ...
  const tournamentStartTime =
    window.tournamentStartTime || Math.floor(Date.now() / 1000) - 3600; // Fallback to 1 hour ago
  ...
  ongoingMatches.forEach((match) => {
    ...
    const player1AcceptedSubmission = player1Submissions.find(
      (sub) =>
    ...
    const player2AcceptedSubmission = player2Submissions.find(
      (sub) =>
        sub.problem_id === match.problem.id &&
        sub.result === "AC" &&
        sub.epoch_second >= tournamentStartTime
    ...
    if (player1AcceptedSubmission && !match.player1Solved) {
      ...
      match.player1Time =
        player1AcceptedSubmission.epoch_second - tournamentStartTime;
      matchesUpdated = true;
    }
    ...
    if (player2AcceptedSubmission && !match.player2Solved) {
      ...
      match.player2Time =
        player2AcceptedSubmission.epoch_second - tournamentStartTime;
      matchesUpdated = true;
    }

    // Determine winner if both solved or one solved
    if (
      (match.player1Solved || match.player2Solved) &&
      match.status === "in_progress"
    ) {
...
function checkRoundComplete() {
  const allMatchesComplete = matches.every(
...
async function startNextRound() {
  ...
  const matchesContainer = document.getElementById("matches-container");
  matchesContainer.innerHTML =
    '<div class="loading-message">Setting up next round...</div>';
  ...
  try {
    ...
    matchesContainer.innerHTML =
      '<div class="error-message">Failed to set up next round. Please try again.</div>';
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Warn on reload/close when there is tournament data to preserve.
  // While the browser confirmation is shown, the page state is retained.
  window.addEventListener("beforeunload", function (e) {
    if (players.length > 0 || matches.length > 0) {
      e.preventDefault();
      // Required for most browsers to show the confirmation dialog.
      e.returnValue = "";
      return "";
    }
    // No return -> no prompt if there's no active data
  });
});
// ...existing code...
